    ...
    def test_surrogate_key_filled(self):
        ...
        with engine.connect() as conn:
            nulls = conn.execute(text(
                "SELECT count(*) FROM silver.vigitel_cleansed WHERE sk_registro IS NULL"
            )).scalar()
    ...
    def test_continuous_columns_preserved(self):
        ...
        with engine.connect() as conn:
            count = conn.execute(text(
                "SELECT count(*) FROM silver.vigitel_cleansed WHERE peso_amostral IS NOT NULL"
            )).scalar()
...
class TestGold:
    ...
    def test_dim_tempo_coverage(self):
        ...
        with engine.connect() as conn:
            silver_years = conn.execute(text(
    ...
    def test_dim_cidade_has_names(self):
        ...
        with engine.connect() as conn:
            named = conn.execute(text(
                "SELECT count(*) FROM gold.dim_cidade WHERE nome_cidade IS NOT NULL"
            )).scalar()
    ...
    def test_dim_perfil_has_bands(self):
        ...
        with engine.connect() as conn:
            faixas = conn.execute(text(
                "SELECT DISTINCT faixa_etaria FROM gold.dim_perfil ORDER BY faixa_etaria"
            )).fetchall()
            faixa_names = {row[0] for row in faixas}
        expected = {'18-24', '25-34', '35-44', '45-54', '55-64', '65+'}
        assert expected.issubset(faixa_names), f"Faixas esperadas: {expected}, encontradas: {faixa_names}"
