...
class QueryFilters:
    ...
    def __init__(
        self,
        ano: Optional[List[int]] = Query(
            None, description="Filtrar por anos específicos (múltiplos valores permitidos)"
        ),
        cidade: Optional[List[str]] = Query(
            None, description="Filtrar pelo nome da cidade (ex: São Paulo, Rio de Janeiro)"
        ),
        sexo: Optional[str] = Query(None, description="Filtrar por sexo (masculino ou feminino)"),
        faixa_etaria: Optional[List[str]] = Query(
            None, description="Filtrar por faixas etárias (ex: 18-24, 25-34)"
        ),
        escolaridade: Optional[List[str]] = Query(
            None, description="Filtrar por faixas de escolaridade (ex: 0-8 anos, 12+ anos)"
        ),
