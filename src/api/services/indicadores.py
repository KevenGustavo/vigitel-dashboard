...
FAIXAS_ETARIAS_ORDEM = {"18-24": 1, "25-34": 2, "35-44": 3, "45-54": 4, "55-64": 5, "65+": 6}
...
def build_proportion_expr(*columns):
    ...
    den = func.sum(
        case((coalesced_col.isnot(None), fato_atividade_fisica.c.peso_amostral), else_=0)
    )

    return case((den > 0, cast((num / den) * 100, Numeric(10, 2))), else_=None)
...
def get_atividade_fisica_columns() -> Tuple[Label, ...]:
    ...
    return (
        build_proportion_expr(
            fato_atividade_fisica.c.ind_ativo_lazer, fato_atividade_fisica.c.ind_ativo_lazer_150min
        ).label("ativo_lazer"),
        build_proportion_expr(fato_atividade_fisica.c.ind_ativo_transporte).label(
            "ativo_deslocamento"
        ),
        build_proportion_expr(fato_atividade_fisica.c.ind_ativo_ocupacional).label(
            "ativo_ocupacional"
        ),
        build_proportion_expr(fato_atividade_fisica.c.ind_ativo_domestico).label("ativo_domestico"),
        build_proportion_expr(
            fato_atividade_fisica.c.ind_inativo_total, fato_atividade_fisica.c.ind_inativo_lazer
        ).label("inativo_total"),
        build_proportion_expr(
...
def get_sedentarismo_columns() -> Tuple[Label, ...]:
    ...
    return (
        build_proportion_expr(fato_atividade_fisica.c.ind_tela_total_maior_3h).label(
            "tempo_tela_maior_3h"
        ),
        build_proportion_expr(fato_atividade_fisica.c.ind_tv_maior_3h).label("tempo_tv_maior_3h"),
        build_proportion_expr(fato_atividade_fisica.c.ind_tela_s_tv_maior_3h).label(
            "tempo_tela_exceto_tv_maior_3h"
        ),
    )
...
async def get_kpi_atividade_fisica(
...
async def get_evolucao_atividade_fisica(
    db: AsyncSession, filters: QueryFilters
) -> List[EvolucaoAtividadeFisica]:
    ...
    query = (
        query.with_only_columns(
            dim_tempo.c.ano_coleta.label("ano"), *get_atividade_fisica_columns()
        )
        .group_by(dim_tempo.c.ano_coleta)
        .order_by(dim_tempo.c.ano_coleta)
    )
...
async def get_evolucao_sedentarismo(
    db: AsyncSession, filters: QueryFilters
) -> List[EvolucaoSedentarismo]:
    ...
    query = apply_filters(get_base_query(), filters)
    query = (
        query.with_only_columns(dim_tempo.c.ano_coleta.label("ano"), *get_sedentarismo_columns())
        .group_by(dim_tempo.c.ano_coleta)
        .order_by(dim_tempo.c.ano_coleta)
    )
...
async def get_evolucao_desfechos(
    db: AsyncSession, filters: QueryFilters
) -> List[EvolucaoDesfechosSaude]:
    ...
    query = apply_filters(get_base_query(), filters)
    query = (
        query.with_only_columns(dim_tempo.c.ano_coleta.label("ano"), *get_desfechos_columns())
        .group_by(dim_tempo.c.ano_coleta)
        .order_by(dim_tempo.c.ano_coleta)
    )
...
async def get_comparativo_sexo(db: AsyncSession, filters: QueryFilters) -> ComparativoSexoResponse:
    ...
    query = query.with_only_columns(
        dim_perfil.c.sexo.label("sexo"), *get_atividade_fisica_columns()
...
async def get_sedentarismo_faixa_etaria(
    db: AsyncSession, filters: QueryFilters
) -> List[SedentarismoFaixaEtariaItem]:
    ...
    query = query.with_only_columns(
        dim_perfil.c.faixa_etaria.label("faixa_etaria"), *get_sedentarismo_columns()
...
async def get_desfechos_cidades(
    db: AsyncSession, filters: QueryFilters, indicador: str = "obesidade"
) -> List[RankingCidadeItem]:
    ...
    sorted_rows = sorted(
        rows, key=lambda r: float(r["valor"]) if r["valor"] is not None else -1.0, reverse=True
    )
...
async def get_dashboard_consolidado(
    db: AsyncSession, filters: QueryFilters, indicador_ranking: str = "obesidade"
) -> DashboardConsolidadoResponse:
    """
