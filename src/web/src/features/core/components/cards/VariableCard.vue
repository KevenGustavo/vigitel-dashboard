<template>
  <div
    class="bg-card border border-border rounded-xl p-3 shadow-xs relative group/card hover:border-stone-300 transition-all hover:shadow-xs hover:z-30"
    :class="extraClass"
  >
    ...
    <div class="flex items-center justify-between text-xs text-text-secondary mb-1">
      <div class="flex items-center gap-1.5 min-w-0 pr-1">
        ...
        <span class="font-medium truncate font-display text-text-primary text-[11px] sm:text-xs" :title="label">
          {{ label }}
        </span>
      </div>

      <!-- Botão / Trigger de Informação com Popover Hover -->
      <div class="relative group/info shrink-0 cursor-help">
        <div class="w-4.5 h-4.5 rounded-full bg-stone-100 hover:bg-stone-200/80 border border-stone-200/90 flex items-center justify-center text-text-secondary transition-colors shadow-2xs">
        ...
        <div
          class="absolute top-full mt-2 w-64 sm:w-72 p-3 bg-stone-900 text-stone-100 rounded-lg shadow-xl border border-stone-700 text-xs opacity-0 invisible group-hover/info:opacity-100 group-hover/info:visible transition-all duration-200 z-50 pointer-events-none text-left"
    ...
    <div class="text-lg font-bold text-text-primary font-mono tracking-tight">
      <div v-if="isLoading" class="h-6 w-16 bg-stone-200 animate-pulse rounded my-0.5"></div>
      <span v-else>{{ formattedValue }}</span>
    </div>

    <!-- Base: Subtítulo Explicativo Curto -->
    <div class="text-[10px] text-text-muted mt-0.5 truncate" :title="subtitle">
      {{ subtitle }}
    </div>
  </div>
</template>
